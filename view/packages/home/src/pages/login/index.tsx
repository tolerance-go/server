import { PUBLIC_PATH } from '@fenxing/common/constants/base';
import { HOST_ORIGINS } from '@fenxing/common/constants/hosts';
import { PATHS, QUERY_KEYS } from '@fenxing/common/constants/path';
import { STORE_TAG } from '@fenxing/common/constants/store';
import { getPublicKey } from '@fenxing/common/helpers/getPublicKey';
import {
  UserControllerCreate,
  UserControllerLogin,
} from '@fenxing/common/services/server/UserController';
...
export default () => {
  ...
  const login = useMemoizedFn(async (username: string, password: string) => {
    ...
    const validatePathname = (pathname: string) => {
      try {
        new URL(`http://www.dump.com${pathname}`);
        return true;
      } catch {
        return false;
      }
    ...
    window.location.href = `${HOST_ORIGINS.ADMIN}${
      quitFrom && validatePathname(quitFrom) ? quitFrom : PATHS.HOME
    }`;
  });
